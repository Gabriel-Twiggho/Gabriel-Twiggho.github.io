...
replaceExact(
  ...
  "var hoverNext=hover?WCKO.next[hover]:null;Rt.useLayoutEffect(function(){function fit(){var w=wrapRef.current,inr=innerRef.current;if(!w||!inr)return;var cw=w.clientWidth,sw=inr.scrollWidth;if(!cw||!sw)return;var s=Math.min(1,cw/sw);if(s<.5)s=.5;setDims(function(p){var nh=inr.offsetHeight*s;return(Math.abs(p.s-s)<.002&&Math.abs(p.h-nh)<1)?p:{s:s,h:nh};});}fit();var t=setTimeout(fit,120);var ro=null;if(window.ResizeObserver){ro=new ResizeObserver(fit);wrapRef.current&&ro.observe(wrapRef.current);}window.addEventListener(\"resize\",fit);return function(){clearTimeout(t);window.removeEventListener(\"resize\",fit);ro&&ro.disconnect();};},[U,focus]);"
);
replaceExact(
  ...
  "X.jsx(\"div\",{className:\"bracket-scroll\",ref:wrapRef,style:{height:dims.h?dims.h+\"px\":void 0,overflow:\"hidden\"},children:X.jsxs(\"div\",{className:\"wc-ko\"+(focus?\" has-focus\":\"\"),ref:innerRef,style:{transform:\"scale(\"+dims.s+\")\",transformOrigin:\"top center\"},children:["
);
